...
const Organization = bookshelf.model('Organization', {
  ...
  validateCreate: Promise.method(function (model, attrs) {
    ...
    return GithubAPI.isOrganization(attrs.github_id)
      .then(org => {
        if (!org) {
  ...
  fetchById: Promise.method(function (githubId) {
    const log = logger.child({ githubId: githubId, method: 'Orgainization.fetchById' })
