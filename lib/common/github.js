...
module.exports = class GithubAPI {
  ...
  static getOrganization (githubId) {
    return Promise.fromCallback(cb => {
      ...
      .then(function (org) {
        if (!org) throw GithubEntityNotFoundError('Github organization not found.', { githubId: githubId })
        if (org.type !== 'Organization') {
          throw GithubEntityTypeError('Github entity is not a organization', { res: org, githubId: githubId })
        }
        return org
      })
  }
  ...
  static getUser (githubId) {
    return Promise.fromCallback(cb => {
      ...
      .then(function (user) {
        if (!user) throw GithubEntityNotFoundError('User not found', { githubId: githubId })
        if (user.type !== 'User') {
          throw GithubEntityTypeError('Entity is not a user', { res: user, githubId: githubId })
        }
        return user
      })
  }
