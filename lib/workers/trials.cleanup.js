    ...
    .then((companies) => {
      return Promise.all(companies.map(company => {
        return Bookshelf.transaction(t => {
          if (!company.orgId) {
            log.info('Intercom company missing github orgId')
            return
          }
          return Organization.fetchByGithubId(company.orgId, { transacting: t, forUpdate: true })
            .then(res => {
              log.info({ orgId: company.orgId, orgName: company.name }, 'Disabling Runnable trial org.')
              return res.org.save({ isActive: false }, { transacting: t, patch: true })
            })
        })
      }))
    })
