 ...
 */
module.exports = function OrganizationAuthorized (rawJob) {
  const log = logger.child({ job: rawJob, method: 'OrganizationAuthorized' })
  return Joi.validateAsync(rawJob, jobSchema, { stripUnknown: true })
    .then(job => {
      log.info('OrganizationAuthorized called')
