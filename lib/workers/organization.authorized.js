 ...
 */
module.exports.task = function OrganizationAuthorized (job) {
  ...
  return User.fetchByGithubId(job.creator.githubId)
    ...
    .tap(function publishOrganizationCreatedEvent (org) {
      const orgName = org.get('name')
      const githubId = org.get('githubId')
      const isPersonalAccount = org.get('isPersonalAccount')
      log.trace({
        githubId: githubId,
        orgName: orgName
