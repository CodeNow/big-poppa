...
module.exports = function CreateOrganization (rawJob) {
  ...
  return Joi.validateAsync(rawJob, jobSchema)
    .catch(err => {
      log.trace({ err: err }, 'Error validating stuff')
      throw err
    })
    .then(job => {
    ...
    .catch(err => {
      log.trace({ err: err, keys: Object.keys(err), isJoi: err.isJoi }, 'Error processing stuff')
      if (err.isJoi) {
        log.trace('THROW WORKER STOP ERROR')
        throw new WorkerStopError(
          `Invalid Job: ${err.toString()}`,
          { err: err }
        )
      }
      log.trace('throw normal error')
      throw err
    })
}
