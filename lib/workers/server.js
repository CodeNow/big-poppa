'use strict'

const ErrorCat = require('error-cat')
const log = require('util/logger').child({ module: 'worker-server' })
const ponos = require('ponos')

/**
 * The big-poppa ponos server.
 * @type {ponos~Server}
 * @module big-poppa/worker
 */
module.exports = new ponos.Server({
  name: process.env.APP_NAME,
  rabbitmq: {
    channel: {
      prefetch: process.env.RABBITMQ_PREFETCH
    },
    hostname: process.env.RABBITMQ_HOSTNAME,
    port: process.env.RABBITMQ_PORT,
    username: process.env.RABBITMQ_USERNAME,
    password: process.env.RABBITMQ_PASSWORD
  },
  errorCat: ErrorCat,
  log: log,
  tasks: {
    'organization.create': require('./organization.create'),
    'organization.delete': require('./organization.delete'),
    'user.create': require('./user.create-or-update'),
    'user.delete': require('./user.delete'),
    'organization.user.add': require('./organization.user.add'),
    'organization.user.remove': require('./organization.user.remove')
  },
  events: {
    'organization.authorized': require('./organization.create'),
    'organization.integration.runnabot.disabled': require('./organization.integration.runnabot.disabled'),
    'organization.integration.runnabot.enabled': require('./organization.integration.runnabot.enabled'),
    'organization.user.added': require('./organization.user.added'),
    'user.authorized': require('./user.create-or-update')
  }
})
