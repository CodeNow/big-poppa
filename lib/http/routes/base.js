...
module.exports = class BaseRouter {
  ...
  static errorHandler (res, err) {
    ...
    if (err instanceof RegistryDoesNotSupportLoginError) {
      log.error('Registry Does not support login Error')
      return res.status(404).json({ statusCode: 404, message: err.toString(), err: err.message })
    }
    if (err instanceof UnauthorizedError) {
      log.error('Unauthorized Error')
      return res.status(401).json({ statusCode: 401, message: err.toString(), err: err.message })
    }
