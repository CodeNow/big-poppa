...
module.exports = class BaseRouter {
  ...
  static errorHandler (res, err) {
    const log = logger.child({ method: 'errorHandler', errMessage: err.message })
    log.info('errorHandler called')
    if (err instanceof NotFoundError) {
      log.trace('404 NotFoundError')
      return res.status(404).json({ statusCode: 404, message: 'Resource Not Found', err: err.message })
    }
    if (err.isJoi) {
      log.trace('400 Validation Error')
      return res.status(400).json({ statusCode: 400, message: 'Validation Error', err: err.message })
    }
    ...
    if (
      ...
      log.trace('400 Caught exception')
      return res.status(400).json({ statusCode: 400, message: err.name, err: err.message })
    }
