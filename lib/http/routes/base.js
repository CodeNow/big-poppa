...
module.exports = class BaseRouter {
  ...
  static createRoute (routerFunction, schema) {
    ...
    return Promise.method(function () {
      ...
      /**
       * Validate the schema twice so that we can catch any unknown properties
       * being passed into a query.
       *
       * Unfortunately, we need to do this twice because `Joi` ignores checking
       * for unknown properties when the `stripUnknown` flag is passed
       */
      return Promise.join(
        ...
        .tap(request => res.set('ValidatedRequest', JSON.stringify(request)))
