...
class OrganizationRouter extends BaseRouter {
  ...
  static patchOne (validatedReq, res) {
    let validatePrivateRegistryPromise = Promise.resolve()
    if (validatedReq.body.privateRegistryUrl &&
      validatedReq.body.privateRegistryUsername &&
      validatedReq.body.privateRegistryPassword) {
      validatePrivateRegistryPromise = DockerRegistry.validateCredentials(
        validatedReq.body.privateRegistryUrl,
        validatedReq.body.privateRegistryUsername,
        validatedReq.body.privateRegistryPassword
      )
    }
    delete validatedReq.body.privateRegistryPassword
    return validatePrivateRegistryPromise
      .then(() => {
