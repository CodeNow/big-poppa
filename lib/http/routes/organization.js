...
class OrganizationRouter extends BaseRouter {
  ...
  static sendOrgResponse (org, res) {
    var toJSON = Promise.method(org.toJSON.bind(org), null)
    return toJSON()
      .then((org) => {
        return OrganizationRouter.transformSingleOrg(org)
      })
      .then(org => {
        if (org.users) {
          return OrganizationRouter.removeUserAccessTokens(org)
        } else {
  ...
  static patchOne (validatedReq, res) {
    return Organization.fetchById(validatedReq.params.id)
      .then(org => {
        if (validatedReq.body.metadata && org.attributes) {
