...
class OrganizationRouter extends BaseRouter {
  ...
  static transformSingleOrg (org) {
    ...
    Object.assign(outputOrg, org, {
      ...
      isOnPrem: process.env.ON_PREM === 'true'
    })
    return Object.assign(outputOrg, {
      ...
      allowed: outputOrg.isActive && !outputOrg.isPermanentlyBanned && (outputOrg.isInTrial || outputOrg.isInActivePeriod || gracePeriodEnd.isAfter(now))
    })
  }
   ...
   * @param {String}           validatedReq.body.privateRegistryUrl      - Url of the private registry
   * @param {String}           validatedReq.body.privateRegistryUsername - Username to access the private registry
   * @param {String}           validatedReq.body.privateRegistryPassword - Password to access the private registry
   *
  ...
  static patchOne (validatedReq, res) {
    return Bookshelf.transaction(transacting => {
      return Organization.fetchById(validatedReq.params.id, { transacting, forUpdate: true })
        .then(org => {
          return Organization.processDockerRegistryCredentials(validatedReq.params.id, validatedReq.body)
            .then(orgPatchParams => {
              org.on('saving', (model, attrs, opts) => {
                res.set('Model-Updates', JSON.stringify(model.changed))
              })
              if (orgPatchParams.metadata) {
                orgPatchParams.metadata = Object.assign(org.attributes.metadata || {}, orgPatchParams.metadata)
              }
              return org.save(orgPatchParams, { transacting, patch: true })
            })
            // Re-fetch org in order to apply any database triggers
            .then(() => Organization.fetchById(validatedReq.params.id, { transacting }))
        })
    })
    .then(org => OrganizationRouter.sendOrgResponse(org, res))
  }
...
OrganizationRouter.patchSchema = Joi.object({
  ...
  body: Joi.object({
    ...
    hasPaymentMethod: Joi.boolean(),
    isActive: Joi.boolean(),
    isPermanentlyBanned: Joi.boolean(),
    metadata: Joi.object({
      hasAha: Joi.boolean(),
      hasCompletedDemo: Joi.boolean(),
      hasConfirmedSetup: Joi.boolean()
    }),
    prBotEnabled: Joi.boolean(),
    privateRegistryUrl: Joi.string(),
    privateRegistryUsername: Joi.string(),
    privateRegistryPassword: Joi.string(),
