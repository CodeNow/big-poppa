...
class OrganizationRouter extends BaseRouter {
  ...
  static router () {
    ...
    router.patch('/:id/metadata/', OrganizationRouter.createRoute(OrganizationRouter.patchMetadata, OrganizationRouter.patchSchema))
    return router
  }
   ...
   *
   * @param {Number}            id                 - Organization model instance ID
   * @resolves {Organization}                      - An Organization model instance
   * @return {Promise}
   */
  static refetchOrg (id, res) {
    return Organization.fetchById(id)
      .then(org => org.toJSON())
  ...
  static patchOne (validatedReq, res) {
    return Organization.fetchById(validatedReq.params.id)
      ...
      .then(() => OrganizationRouter.refetchOrg(validatedReq.params.id, res))
  }

  /**
   * Modify an `Organization`'\'s metadata by its ID
   *
   * @param {Object}           validatedReq           - Validated request against `getSchema`
   * @param {Object}           validatedReq.params    - Parameters passed in URL
   * @param {Number}           validatedReq.params.id - Organization model instance ID
   * @param {Object}           validatedReq.body      - Body of request with modifications to model's metadata
   * @resolves {Organization}                         - An Organization model instances
   * @return {Promise}
   */
    static patchMetadata (validatedReq, res) {
    return Organization.fetchById(validatedReq.params.id)
      .then(org => {
        if (validatedReq.body.metadata) {
          validatedReq.body.metadata = Object.assign(org.attributes.metadata || {}, validatedReq.body.metadata)
        }
        return org.save(validatedReq.body)
      })
      // Re-fetch org in order to apply any database triggers
      .then(() => OrganizationRouter.refetchOrg(validatedReq.params.id, res))
  }
