...
class OrganizationRouter extends BaseRouter {
  ...
  static patchOne (validatedReq, res) {
    const body = validatedReq.body
    return Bookshelf.transaction(transacting => {
      return Organization.fetchById(validatedReq.params.id, { transacting: transacting, forUpdate: true })
        .tap(org => Organization.validateDockerRegistryCredentials(validatedReq.params.id, validatedReq.body))
        .then(org => {
          ...
          return org.save(body, { transacting: transacting, patch: true })
        })
