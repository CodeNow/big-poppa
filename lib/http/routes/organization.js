...
class OrganizationRouter extends BaseRouter {
  ...
  static get (validatedReq, res) {
    const query = validatedReq.query
    ...
    const queryBuilder = (qb) => {
      Object.keys(query).forEach(key => {
        ...
        switch (specialQueryKey) {
          case 'isNull':
            if (query[key]) {
              qb.whereNull(snakeCaseKey, query[key])
            } else {
              qb.whereNotNull(snakeCaseKey, query[key])
            }
            break
          case 'moreThan':
            qb.andWhere(snakeCaseKey, '>', query[key])
            break
          case 'lessThan':
            qb.andWhere(snakeCaseKey, '<', query[key])
            break
    ...
    return Organization.collection()
      .query(queryBuilder).fetch({ withRelated: 'users' })
  ...
  static patchOne (validatedReq, res) {
    return Organization.fetchById(validatedReq.params.id)
      ...
      .then(() => Organization.fetchById(validatedReq.params.id))
      .then(org => OrganizationRouter.sendOrgResponse(org, res))
  }
...
OrganizationRouter.getSchema = Joi.object({
  query: Joi.object({
    ...
    'stripeCustomerId.isNull': Joi.boolean(),
    'trialEnd.moreThan': Joi.date().iso(),
    'trialEnd.lessThan': Joi.date().iso()
  })
}).unknown()
