...
const Organization = BaseModel.extend('Organization', {
  ...
  getCurrentGracePeriodEnd: function () {
    ...
    if (this.get('gracePeriodEnd')) {
      // If grace period is set in the database, just return it
      return this.get('gracePeriodEnd')
    }
    ...
    if (trialEnd.isAfter(activePeriodEnd)) {
      currentGracePeriodEnd = trialEnd.clone()
    } else {
      currentGracePeriodEnd = activePeriodEnd.clone()
    }
    currentGracePeriodEnd.add(process.env.GRACE_PERIOD_DURATION_IN_HOURS, 'hours')
    log.trace({
