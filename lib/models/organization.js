...
const Organization = BaseModel.extend('Organization', {
  ...
  serialize: function () {
    let serializeModel = bookshelf.Model.prototype.serialize.apply(this, arguments)
    return Object.assign(serializeModel, {
      gracePeriodEnd: this.getCurrentGracePeriodEnd()
    })
  },
  ...
  /**
   * Get time at which grace period will end
   *
   * @returns {Number} - UNIX timestamp
   */
  getCurrentGracePeriodEnd: function () {
    const log = logger.child({
      organizationGithubId: this.get(this.idAttribute),
      method: 'Orgainization.getCurrentGracePeriodEnd'
    })
    log.info('Organization.getCurrentGracePeriodEnd call')
    let currentGracePeriodEnd
    if (this.get('gracePeriodEnd')) {
      currentGracePeriodEnd = moment(this.get('gracePeriodEnd'))
    }
    let activePeriodEndTimestamp = this.get('activePeriodEnd')
    let trialEndTimestamp = this.get('trialEnd')
    let trialEnd = moment(trialEndTimestamp)
    let activePeriodEnd = moment(activePeriodEndTimestamp)
    let newGracePeriodEnd
    if (trialEnd.isAfter(activePeriodEnd)) {
      newGracePeriodEnd = trialEnd.clone()
    } else {
      newGracePeriodEnd = activePeriodEnd.clone()
    }
    newGracePeriodEnd.add(process.env.GRACE_PERIOD_DURATION_IN_HOURS, 'hours')
    // Only keep current grace period if its an extension of current grace period
    if (!currentGracePeriodEnd || newGracePeriodEnd.isAfter(currentGracePeriodEnd)) {
      currentGracePeriodEnd = newGracePeriodEnd
    }
    log.trace({
      currentGracePeriodEnd: currentGracePeriodEnd,
      activePeriodEnd: activePeriodEnd,
      trialEnd: trialEnd
    }, 'Updating grace period')
    return currentGracePeriodEnd.toISOString()
  }
