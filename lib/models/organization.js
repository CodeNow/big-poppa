...
const Organization = BaseModel.extend('Organization', {
  ...
  addUser: Promise.method(function (user, opts) {
    ...
    return githubApi.hasUserOrgMembership(this.get('githubId'))
      .then(() => {
   ...
   * @return {Promise}
   */
  create: Promise.method((githubId, githubName, opts) => {
    const log = logger.child({ githubId: githubId, method: 'Organization.create' })
