...
const Organization = BaseModel.extend('Organization', {
  ...
  updateGracePeriod: Promise.method(function (opts) {
    const log = logger.child({
      organizationGithubId: this.get(this.idAttribute),
      opts: opts,
      method: 'Orgainization.updateGracePeriod'
    })
    log.info('Organization.updateGracePeriod call')
    ...
    if (trialEnd.isAfter(activePeriodEnd)) {
      newGracePeriodEnd = trialEnd.clone().add(72, 'hours')
    } else {
      newGracePeriodEnd = activePeriodEnd.clone().add(72, 'hours')
    }
    log.trace({
      newGracePeriodEnd: newGracePeriodEnd,
      activePeriodEnd: activePeriodEnd,
      trialEnd: trialEnd
    }, 'Updating grace period')
    return this.save({ gracePeriodEnd: newGracePeriodEnd.toDate() })
  })
