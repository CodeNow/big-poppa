...
const Organization = BaseModel.extend('Organization', {
  ...
  validateCreate: function (model, attrs) {
    ...
    return githubApi.getOrganization(attrs.githubId)
      ...
      .catch(GithubEntityTypeError, err => {
        ...
        .then(user => {
            // for a new personal account user, we need to set the isPersonalAccount property to true and enable PR bot
          model.set({ name: user.login, isPersonalAccount: true, prBotEnabled: true, firstDockCreated: true })
  ...
  create: Promise.method((githubId, creator, opts) => {
    ...
    const activePeriodEnd = process.env.ON_PREM === 'true' ? moment().add(1, 'year').utc().toDate() : moment().utc().toDate()
    const firstDockCreated = process.env.ON_PREM === 'true'
    return new Organization()
      .save({
        // By default all newly created orgs should be inactive, except for on_prem
        githubId,
        activePeriodEnd,
        firstDockCreated,
  ...
  /**
   * Check if the registry credentials are valid.  If they are, publish the password in the
   * `org.registry.password.submitted` event, and the opts (without the password) is resolved.
   *  Otherwise, just resolve.
   *
   * @param {Number} orgId                        - BP Organization id
   * @param {Object} opts                         - Opts from the request
   * @param {String} opts.privateRegistryUrl      - Url of the private registry
   * @param {String} opts.privateRegistryUsername - Username to access the private registry
   * @param {String} opts.privateRegistryPassword - Password to access the private registry
   *
   * @resolves {Object} opts                         - Opts from the request, minus the
   *           {String} opts.privateRegistryUrl      - Url of the private registry
   *           {String} opts.privateRegistryUsername - Username to access the private registry
   */
  processDockerRegistryCredentials: (orgId, opts) => {
    const log = logger.child({
      hasPrivateRegistryUrl: !!opts.privateRegistryUrl,
      hasPrivateRegistryUsername: !!opts.privateRegistryUsername,
      hasPrivateRegistryPassword: !!opts.privateRegistryPassword,
      method: 'processDockerRegistryCredentials'
    })
    log.trace('called')
    return Promise.try(() => {
      if (!opts.privateRegistryUrl || !opts.privateRegistryUsername || !opts.privateRegistryPassword) {
        return
      }
      return DockerRegistry.validateCredentials(
        opts.privateRegistryUrl,
        opts.privateRegistryUsername,
        opts.privateRegistryPassword
      )
        .tap(() => rabbitMQ.publishOrgRegistryPasswordSubmitted(orgId, opts.privateRegistryPassword))
        .tap(() => { delete opts.privateRegistryPassword })
    })
      .return(opts)
  },
